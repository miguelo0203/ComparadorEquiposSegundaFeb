# ==============================================================================
# AUDITORÍA Y TEST DE REACTIVIDAD / DDL / DATOS PARA APP.R (DUAL ENGINE)
# ==============================================================================
...
get_db_con <- function() {
  if (file.exists("segunda_feb_pro.sqlite")) {
    ...
    return(con)
  } else if (file.exists("../segunda_feb_pro.sqlite")) {
    con <- dbConnect(RSQLite::SQLite(), "../segunda_feb_pro.sqlite")
    return(con)
  } else {
    con <- dbConnect(
...
con <- get_db_con()
equipos <- dbGetQuery(con, "SELECT id_equipo, nombre_oficial FROM equipos ORDER BY nombre_oficial;") %>%
  mutate(id_equipo = as.numeric(id_equipo))

# 1. Test Tab 1: Team Matchup Engine
cat("[TEST 1/3] Probando consulta Team Matchup (Tab 1)...\n")
id_t1 <- equipos$id_equipo[1]
id_t2 <- equipos$id_equipo[2]

df_loc <- dbGetQuery(con, sprintf("SELECT AVG(pace) AS pace, AVG(ortg) AS ortg, AVG(drtg) AS drtg, AVG(net_rating) AS net_rating, AVG(efg_pct) AS efg_pct FROM team_advanced_stats WHERE id_equipo = %d;", as.integer(id_t1)))
...
cat("[TEST 2/3] Probando consulta Player Scouting (Tab 2)...\n")
df_bio <- dbGetQuery(con, "SELECT j.id_jugador, j.nombre_completo, COALESCE(j.puesto_posicion, 'Posición Desconocida') AS pos, pa.nombre_arquetipo FROM jugadores j LEFT JOIN player_archetypes pa ON j.id_jugador = pa.id_jugador LIMIT 1;")
df_bio <- df_bio %>% mutate(id_jugador = as.numeric(id_jugador))

df_stats <- dbGetQuery(con, sprintf("SELECT AVG(pctil_valoracion) AS val, AVG(pctil_ts_pct) AS ts, AVG(pctil_puntos) AS pts, AVG(pctil_rebotes) AS reb, AVG(pctil_asistencias) AS ast FROM player_advanced_stats WHERE id_jugador = %d;", as.integer(df_bio$id_jugador[1])))
...
cat("[TEST 3/3] Probando consulta Buscador (Tab 3)...\n")
df_rec <- dbGetQuery(con, "
  ...
  WHERE pas.cumple_umbral = 1 AND pa.cluster_id IN (1,2,3,4,5,6)
